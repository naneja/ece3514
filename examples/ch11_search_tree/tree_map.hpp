...
/*
TreeMap class in C++. It’s a map (or dictionary) built on a binary search tree structure, allowing storage of key-value pairs in sorted order and supporting efficient searching, insertion, and deletion.

Main Components:
    TreeMap:
        Stores key-value pairs (Key, Value) in a tree.
        Inherits from AbstractMap<Key, Value>.
        Uses Compare (default is std::less) to order the keys.

    BalanceableBinaryTree:
        Nested class for managing the tree structure.
        Supports rotation and restructuring to help with balancing (balancing functions are empty in this code — placeholders).

    Node & Position:
    Internal tree nodes store pairs (Entry, int), where the integer is used for balancing (e.g., height or balance factor).

    Iterators:
        iter_rep defines how iteration over the map entries works using pointers to nodes.
Members
    less_than — comparator for ordering keys.
    tree — the actual tree storing the data.
    Nodes store BSTEntry = pair<Entry, int> where:
        Entry is a key-value pair.
        int is a placeholder for balancing info.
*/
