...
class CostPerformanceDatabase {
  ...
  public:
    ...
    const_iterator best(int c) const {
        // least key strictly greater than c
        const_iterator here{database.upper_bound(c)}; 
        ...
        //updated
        // find the predecessor of `here` using only ++
        const_iterator prev = database.begin();
        for (auto it = database.begin(); it != here; ++it)
            prev = it;
        return prev;
    }
